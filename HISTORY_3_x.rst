...
- Update Docker deploy script. Use alpine PostgreSQL version.
...
- Add ``PermissionTestCaseMixin`` and ``SmartSettingTestCaseMixin`` to
  better organize cache invalidation of both apps for tests.
- Add a version attribute to setting namespace. These are dumped
  as ``SMART_SETTINGS_NAMESPACES``.
- Add a ``savesettings`` command.
