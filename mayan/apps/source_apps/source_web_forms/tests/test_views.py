...
class WebFormSourceBackendDocumentUploadWizardViewTestCase(
...
class WebFormDocumentCreateSourceSwitchLinkViewTestCase(
    ...
    def test_source_switch_link_with_access(self):
        ...
        self.assertNotContains(
            response=response, text=self._test_sources[0].label,
            status_code=200
        )
        self.assertContains(
            response=response, text=self._test_sources[1].label,
            status_code=200
        )
