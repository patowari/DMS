...
from ..event_prune_backends import (
...
class EventLogPruneBackendLatestTestCase(
    EventLogPruneBackendTestMixin, BaseTestCase
):
    _test_event_prune_backend_class = EventLogPruneBackendLatest
    _test_event_prune_backend_kwargs = {'number': 3}

    def test_backend(self):
        self.assertEqual(
            Action.objects.count(), self._test_event_count - 9
        )
        ...
        self.assertEqual(
            ...
            self._test_object_1_event_count - 3
        )
...
class EventLogPruneBackendLatestPerObjectTestCase(
    EventLogPruneBackendTestMixin, BaseTestCase
):
    _test_event_prune_backend_class = EventLogPruneBackendLatestPerObject
    _test_event_prune_backend_kwargs = {'number': 3}

    def test_backend(self):
        self.assertEqual(
            Action.objects.count(), self._test_event_count - 6
        )

        self.assertEqual(
            self._test_object_list[0].target_actions.count(),
            self._test_object_0_event_count - 3
        )
...
class EventLogPruneBackendLatestPerObjectEventTypeTestCase(
    EventLogPruneBackendTestMixin, BaseTestCase
):
    _test_event_prune_backend_class = EventLogPruneBackendLatestPerObjectEventType
    _test_event_prune_backend_kwargs = {'number': 3}

    def test_backend(self):
        self.assertEqual(
            Action.objects.count(), self._test_event_count - 2
        )

        self.assertEqual(
            self._test_object_list[0].target_actions.count(),
            self._test_object_0_event_count - 1
        )
        ...
        self.assertEqual(
            self._test_object_list[1].target_actions.count(),
            self._test_object_1_event_count - 1
        )
