...
from .mixins import StagingFolderSourceTestMixin


class StagingFolderSourceBackendActionDocumentFileUploadViewTestCase(
    SourceActionViewTestMixin, StagingFolderSourceTestMixin,
...
class StagingFolderSourceBackendActionDocumentUploadViewTestCase(
    SourceActionViewTestMixin, StagingFolderSourceTestMixin,
    GenericDocumentViewTestCase
):
    _test_source_create_auto = False
...
class StagingFolderSourceBackendActionFileDeleteViewTestCase(
    SourceActionViewTestMixin, StagingFolderSourceTestMixin,
    GenericDocumentViewTestCase
):
    _test_source_create_auto = False
    ...
    def _get_test_source_stored_file_list(self):
        return list(
            self._test_source.action_execute(
                interface_name='Model', name='file_list'
            )
        )
    ...
    def test_get_with_access(self):
        ...
        self.assertEqual(staging_folder_file_count, 1)
        self.assertEqual(
            self._get_test_source_stored_file_list()[0]['encoded_filename'],
...
class StagingFolderSourceBackendViewTestCase(
    StagingFolderSourceTestMixin, SourceViewTestMixin,
    GenericDocumentViewTestCase
):
    _test_source_create_auto = False
    auto_upload_test_document = False

    def test_staging_folder_source_create_get_view_no_permission(self):
    ...
    def test_staging_folder_source_create_get_view_with_permission(self):
    ...
    def test_staging_folder_source_create_post_view_no_permission(self):
        source_count = Source.objects.count()
    ...
    def test_staging_folder_source_create_post_view_with_permission(self):
        self.grant_permission(permission=permission_sources_create)
